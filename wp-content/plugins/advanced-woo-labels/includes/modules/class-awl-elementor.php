...
if (!class_exists('AWL_Elementor')) :
    ...
    class AWL_Elementor {
        ...
        public function awl_labels_hooks( $hooks ) {
            ...
            $hooks['before_title']['single']['elementor/widget/render_content'] = array( 'priority' => 10, 'type' => 'filter', 'callback' => array( $this, 'render_content_single_title' ), 'args' => 2 );
        ...
        public function render_content_single_title( $widget_content, $block ) {

            if ( is_singular( 'product' ) && $block->get_name() === 'heading' ) {
                $block_display_settings = $block->get_settings_for_display();
                if ( is_array( $block_display_settings ) && isset( $block_display_settings['__dynamic__'] ) && isset( $block_display_settings['__dynamic__']['title'] ) && strpos( $block_display_settings['__dynamic__']['title'], 'woocommerce-product-title-tag' ) !== false ) {
        ...
        public function render_content_single_image( $widget_content, $block ) {
            ...
            if ( is_singular( 'product' ) && $block->get_name() === 'render_content_single_image' ) {
                $widget_content = AWL_Label_Display::instance()->show_label( 'on_image' ) . $widget_content;

            }

            // Jet Woo Gallery
            if ( is_singular( 'product' ) && $block->get_name() === 'template' && strpos( $widget_content, 'jet-woo-product-gallery') !== false ) {
